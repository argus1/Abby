...
* **Current execution status:** CompDetRAE Phases 0–6, the Part 7 quality gates, and Appendix A RepSeq interoperability are implemented. Available annotations carry numbering aliases, boundary evidence, a deterministic CompDetRAE toolchain parameter hash, and `interop_profile=abby_structural_v1_1`. The deterministic AIRR Standards `v2.0.0` partial-Rearrangement serializer and explicit POST transport endpoint remain disabled in default prediction flow; the endpoint persists a project-scoped artifact and returns a signed reference, while reporting unavailable required repertoire fields instead of fabricating them and preserving mmCIF-derived boundaries as authoritative.
...
* **Current execution status:** the `aptamer_target` mode contract is implemented across backend, OpenAPI, frontend, prediction, and batch surfaces. The validation profile classifies DNA/RNA/protein/mixed chains, records canonical and modified nucleotides, enforces aptamer/target partner roles, emits typed mixed/modified warnings, and threads the profile into prediction provenance. Typed compatibility diagnostics identify legacy RNA-prefixed residue aliases and star-prime atom notation without blocking otherwise parseable structures. A bounded Na⁺/Mg²⁺ inventory and non-blocking ionization preflight distinguish coordinate-level ion presence from unknown concentration, neutralization adequacy, and structural-versus-solution ion roles without estimating whole-system charge. Deterministic `aptamer_sasa_fraction` and cutoff-aware `aptamer_counterion_contact_count` descriptors are emitted under `aptamer_features_v2` and covered by the existing feature artifact plus descriptor hash provenance. Synthetic fixtures under `validation_dataset/aptamer/` verify production conversion parity, malformed naming behavior, ionization diagnostics, deterministic descriptor provenance, and explicit phosphodiester connectivity through validation and prediction provenance. Conversion deliberately does not fabricate `_struct_conn` records. The remaining flexibility/broader ion-contact descriptor tranche, broader real-world corpus coverage, and simulation parity remain open.
