...
* **Current execution status:** the `aptamer_target` mode contract is implemented across backend, OpenAPI, frontend, prediction, and batch surfaces. The validation profile classifies DNA/RNA/protein/mixed chains, records canonical and modified nucleotides, enforces aptamer/target partner roles, emits typed mixed/modified warnings, and threads the profile into prediction provenance. Typed compatibility diagnostics identify legacy RNA-prefixed residue aliases and star-prime atom notation without blocking otherwise parseable structures. A bounded Na⁺/Mg²⁺ inventory and non-blocking ionization preflight now distinguish coordinate-level ion presence from unknown concentration, neutralization adequacy, and structural-versus-solution ion roles without estimating whole-system charge. Synthetic fixtures under `validation_dataset/aptamer/` verify production conversion parity, malformed naming behavior, ionization diagnostics, and explicit phosphodiester connectivity through validation and prediction provenance. Conversion deliberately does not fabricate `_struct_conn` records. Aptamer descriptors, broader real-world corpus coverage, and simulation parity remain open.
