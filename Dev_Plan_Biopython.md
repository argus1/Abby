...
* **Slice C (feature parity) — partial:** `aptamer_target` predictions now emit `aptamer_sasa_fraction` (aptamer-partner SASA divided by total partner SASA) under the explicit `aptamer_features_v1` descriptor version. The existing feature artifact and provenance hash carry the value, and a public upload → validation → prediction regression verifies identical descriptors and hashes for identical inputs. Flexibility, ion-contact, and aptamer-specific explainability work remain open.
