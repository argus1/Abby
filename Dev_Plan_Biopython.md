...
* **Slice B (parsing/validation) — complete:** DNA/RNA/protein/mixed chain typing, canonical nucleotide counts, modified-nucleotide records, role validation, mixed/modified warnings, typed legacy residue/atom naming compatibility diagnostics, bounded Na⁺/Mg²⁺ inventory, and non-blocking ionization preflight are implemented. Static coordinates do not establish concentration, neutralization adequacy, or structural-versus-solution ion roles, so the preflight requests review without estimating whole-system charge.
...
* **Slice C (feature parity) — partial:** `aptamer_target` predictions now emit `aptamer_sasa_fraction` (aptamer-partner SASA divided by total partner SASA) and `aptamer_counterion_contact_count` (unique recognized Na⁺/Mg²⁺ residues within the provenanced contact cutoff of the aptamer partner) under the explicit `aptamer_features_v2` descriptor version. Existing feature artifacts and provenance hashes carry both values, and public upload → validation → prediction regressions verify identical hashes for identical inputs. Flexibility, broader ion-contact summaries, and aptamer-specific explainability work remain open.
