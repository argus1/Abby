...
- `dna_aptamer_na_mg_counterions.mmcif` — a minimal DNA-target fixture with one `NA` near the aptamer and one distant `MG`. It exercises typed counterion inventory, the non-blocking ionization-preflight boundary, and unique-ion contact counting at the prediction contact cutoff.
